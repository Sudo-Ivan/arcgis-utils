...
func main() {
	...
	for key, layerInfo := range layersToProcess {
		...
		go func() {
			...
			err := processSelectedLayer(client, layerInfoCopy, formatCopy, outputDirCopy, overwriteCopy, skipExistingCopy, prefixCopy, excludeSymbolsCopy, saveSymbolsCopy, *versionedOutputPtr)
			if err != nil {
...
func processSelectedLayer(client *arcgis.Client, layerInfo arcgis.AvailableLayerInfo, format, outputDir string, overwrite, skipExisting bool, prefix string, excludeSymbols, saveSymbols, versionedOutput bool) error {
	...
	case "kmz":
		...
		if versionedOutput {
			...
			safeFilenameBase = fmt.Sprintf("%s_%s", safeFilenameBase, timestamp)
		}
